...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        public void ExecuteWriteCommand(IConfigItem configItem, ushort transactionId, byte remoteUnitAddress, ushort pointAddress, int value)
        {
            ...
            {
                ValidateDigitalCommand(pointAddress, value);
        ...
        private void ValidateDigitalCommand(ushort pointAddress, int value)
        {
            const ushort valveAddress = 2000;
            const ushort pump1Address = 3000;
            const ushort pump2Address = 3001;
            const ushort pump3Address = 3002;

            bool commandIsOpeningValve = pointAddress == valveAddress && value == 0;
            bool commandIsStartingPump =
                (pointAddress == pump1Address || pointAddress == pump2Address || pointAddress == pump3Address)
                && value == 1;

            if (commandIsOpeningValve && IsAnyPumpRunning())
            {
                throw new InvalidOperationException("Nije dozvoljeno otvoriti ventil dok radi neka od pumpi.");
            }

            if (commandIsStartingPump && IsValveOpen())
            {
                throw new InvalidOperationException("Nije dozvoljeno uključiti pumpu dok je ventil otvoren.");
            }
        }

        private bool IsValveOpen()
        {
            List<IPoint> points = storage.GetPoints(new List<PointIdentifier>
    {
        new PointIdentifier(PointType.DIGITAL_OUTPUT, 2000)
    });

            return points.Count > 0 && points[0].RawValue == 0;
        }

        private bool IsAnyPumpRunning()
        {
            List<IPoint> points = storage.GetPoints(new List<PointIdentifier>
    {
        ...
        new PointIdentifier(PointType.DIGITAL_OUTPUT, 3002)
            ...
            return points.Any(p => p.RawValue == 1);
        }

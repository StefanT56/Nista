    ...
    internal class ConfigItem : IConfigItem
	{
        ...
        public ConfigItem(List<string> configurationParameters)
		{
			...
			if(RegistryType == PointType.ANALOG_INPUT || RegistryType == PointType.ANALOG_OUTPUT){
				if(configurationParameters.Count > 10 && !configurationParameters[10].Equals("#"))
				{
					Double.TryParse(configurationParameters[10], out doubleTemp);
					ScaleFactor = doubleTemp;
				}

                if (configurationParameters.Count > 11 && !configurationParameters[11].Equals("#"))
                {
                    Double.TryParse(configurationParameters[11], out doubleTemp);
                    Deviation = doubleTemp;
                }

                if (configurationParameters.Count > 12 && !configurationParameters[12].Equals("#"))
                {
                    Double.TryParse(configurationParameters[12], out doubleTemp);
                    EGU_Min = doubleTemp;
                }

                if (configurationParameters.Count > 13 && !configurationParameters[13].Equals("#"))
                {
                    Double.TryParse(configurationParameters[13], out doubleTemp);
                    EGU_Max = doubleTemp;
                }

                if (configurationParameters.Count > 15 && !configurationParameters[15].Equals("#"))
                {
                    Double.TryParse(configurationParameters[15], out doubleTemp);
                    HighLimit = doubleTemp;
                }

                if (configurationParameters.Count > 16 && !configurationParameters[16].Equals("#"))
                {
			...
			//Digitalne velicine
			if(RegistryType == PointType.DIGITAL_INPUT || RegistryType == PointType.DIGITAL_OUTPUT)
			{
				if(configurationParameters.Count > 14 && !configurationParameters[14].Equals("#"))
				{
